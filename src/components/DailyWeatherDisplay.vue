...
watch(() => props.cityData, (newCityData) => {
	...
	state.nextDaysForecasts = nextDaysForecasts;
	state.city = city;
}, { immediate: true }
);
...
<template>
	<div class="bg-white p-6 rounded-lg shadow-md mb-4">
		<h2
			class="text-xl font-bold mb-4  pb-4 border-b border-b-sky-800 bg-gradient-to-r from-sky-600 to-sky-950 text-transparent bg-clip-text">
			Next 5 days </h2>

		<div class="flex flex-col justify-between">
			<div v-for="(forecast, index) in state.nextDaysForecasts" :key="forecast.day"
				:class="['flex justify-between items-center px-3', { 'border-b border-sky-200': index !== state.nextDaysForecasts.length - 1 }]">
